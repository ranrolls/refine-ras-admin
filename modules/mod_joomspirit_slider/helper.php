...
class mod_joomspirit_sliderHelper{
	...
	public static function getImages(&$params, $module_id){
		...
		while($i < 23) :
			...
			if ( $params->get('image'.$i.'customlink') ) { $link = "<span class='read-more-slider'><a target='".$params->get('target')."' href='".$params->get('image'.$i.'customlink')."' style='color:".$color_read_more.";'>".$params->get('read_more')."</a></span>" ;} else { $link = "" ;}
			

			// if image + caption + no title
			if ( ($params->get('image'.$i)) && ($params->get('image'.$i.'cap')) && ($params->get('image'.$i.'title') == null) ) {
				
					$listitem = "<li class='".$text_shadow."-text-shadow'><picture><img title='' alt='".$params->get('image'.$i.'alt')."' src='".JURI::root().$params->get('image'.$i)."' /></picture><div class='slide-desc-box ".$position_text." ".$anim_text."' ><div ".$fadeText."><div class='".$bg_text."' style='max-width:".$max_width."px' ><div class='slide-desc' style='color:".$color_text.";font-size:".$font_size_text.";'><span>".$params->get('image'.$i.'cap')."</span>".$link."</div></div></div></div></li>";
			...
			// if image + no caption + no title
					...
					$listitem = "<li class='".$text_shadow."-text-shadow'><picture><img title='' alt='".$params->get('image'.$i.'alt')."' src='".JURI::root().$params->get('image'.$i)."' /></picture></li>";
					array_push($imgsAndCaps, $listitem);
