...
class NSP_GK4_Layout_Parts {
	// header generator
	function header($config, $news_id, $news_cid, $news_title, $lang) {
		if($config['news_content_header_pos'] != 'disabled') {
            ...
            $link = ($news_id !== 0) ? JRoute::_(ContentHelperRoute::getArticleRoute($news_id, $news_cid, $lang)) : JRoute::_('index.php?option=com_user&view=login');
			//
	...
	function text($config, $news_id, $news_cid, $news_text, $news_readmore, $lang) {
	...
	function image($config, $uri, $news_id, $news_iid, $news_cid, $news_text, $news_title, $images, $lang){		
		$news_title = str_replace('"', "&quot;", $news_title);
        $IMG_SOURCE = '';
		$IMG_LINK = ($news_id !== 0) ? JRoute::_(ContentHelperRoute::getArticleRoute($news_id, $news_cid, $lang)) : JRoute::_('index.php?option=com_user&view=login');	
	...
	function readMore($config, $news_id, $news_cid, $lang) {
		//
	...
	function info($config, $news_catname, $news_cid, $news_author, $news_author_email, $news_date, $news_hits, $news_id, $rating_count, $rating_sum, $lang, $num = 1) {
		...
		if(($config['news_content_info_pos'] != 'disabled' && $num == 1) || ($config['news_content_info2_pos'] != 'disabled' && $num == 2)) {
            ...
            //
            $info_category = ($config['category_link'] == 1) ? '<a href="'.(($news_id !== 0) ? JRoute::_(ContentHelperRoute::getCategoryRoute($news_cid, $lang)) : JRoute::_('index.php?option=com_user&view=login')).'" >'.$news_catname.'</a>' : $news_catname;
	...
	function lists($config, $news_id, $news_cid, $news_title, $news_text, $lang, $odd, $num) {
