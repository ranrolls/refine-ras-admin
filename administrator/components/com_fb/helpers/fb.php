...
class FbHelper
{
	 ...
	 */
	public static function addSubmenu($vName = 'fbs')
	{
        	if(version_compare(JVERSION,'3','<')){
		JSubMenuHelper::addEntry(
			JText::_('Fbs'),
			'index.php?option=com_fb&view=fbs',
			($vName == 'fbs')
		);	
	} else {
	...
	public static function getActions($categoryId = 0)
	{
		...
		{
			$assetName = 'com_fb.category.'.(int) $categoryId;
			$level = 'category';
		}
	
		$actions = JAccess::getActions('com_fb', $level);
	...
	public static function getExtensions() 
	{
		...
		$xml = simplexml_load_file(JPATH_ADMINISTRATOR.'/components/com_fb/elements/extensions.xml', 'JXMLElement');		        
	...
	public static function getCategoryViews()
	{
		...
		foreach($extensions as $extension ) {
			$views[$extension->listview->__toString()] = 'com_fb.'.$extension->name->__toString();
		}
		return $views;
	}	
}
...
abstract class JHtmlFb
{
	...
	public static function categories($extension, $cat_id,$name="categories",$title="Select Category", $config = array('attributes'=>'class="inputbox"','filter.published' => array(0,1)))
	{
			...
			$query->from('#__fb_categories AS a');
