...
var JFormValidator = new Class({
	...
	validate: function (el) {
		if (jQuery(el).parents('#field-preview').length > 0) {
			return true;
		}
		el = document.id(el);
	...
	isValid: function (form) {
		...
		if (!valid) {
			var message = Joomla.JText._('COM_JUDIRECTORY_FIELD_INVALID', 'Invalid field: ');
			...
			for (var i = 0; i < errors.length; i++) {
				...
				if (label) {
					...
					invalid_message = jQuery(errors[i]).data("invalid_message");
					if (invalid_message) {
						error.error[i] = invalid_message;
					}
				}
			}
