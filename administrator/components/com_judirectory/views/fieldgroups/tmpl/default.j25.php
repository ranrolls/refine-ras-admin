		...
		<table class="table table-striped adminlist">
			...
			<tr>
				<th style="width:2%" class="center hidden-phone">
					<input type="checkbox" onclick="Joomla.checkAll(this)" title="<?php echo JText::_('COM_JUDIRECTORY_CHECK_ALL'); ?>"
						value="" name="checkall-toggle" />
				</th>
				<th style="width:25%" class="nowrap">
					<?php echo JHtml::_('grid.sort', 'COM_JUDIRECTORY_FIELD_GROUP_NAME', 'fg.name', $listDirn, $listOrder); ?>
				</th>
				<th style="width:40%" class="nowrap">
					<?php echo JText::_('COM_JUDIRECTORY_FIELD_ASSIGNED_CATEGORIES'); ?>
				</th>
				<th style="width:10%" class="nowrap">
					<?php echo JHtml::_('grid.sort', 'COM_JUDIRECTORY_FIELD_TOTAL_FIELDS', 'total_fields', $listDirn, $listOrder); ?>
				</th>
				<th style="width:10%" class="nowrap">
					<?php echo JHtml::_('grid.sort', 'COM_JUDIRECTORY_FIELD_ORDERING', 'fg.ordering', $listDirn, $listOrder); ?>
					<?php if ($saveOrder && $this->groupCanDoManage) : ?>
						<?php echo JHtml::_('grid.order', $this->items, 'filesave.png', 'fieldgroups.saveorder'); ?>
					<?php endif; ?>
				</th>
				<th style="width:10%" class="nowrap">
					<?php echo JHtml::_('grid.sort', 'COM_JUDIRECTORY_FIELD_PUBLISHED', 'fg.published', $listDirn, $listOrder); ?>
				</th>
				<th style="width:3%" class="nowrap">
					<?php echo JHtml::_('grid.sort', 'COM_JUDIRECTORY_FIELD_ID', 'fg.id', $listDirn, $listOrder); ?>
				</th>
			</tr>
			</thead>
				...
				<tr class="row<?php echo $i % 2; ?>">
					...
					<td class="order">
						<?php if ($canChange) : ?>
							<?php if ($saveOrder) : ?>
								<?php if ($listDirn == 'asc') : ?>
									<span><?php echo $this->pagination->orderUpIcon($i, $item->ordering > 2, 'fieldgroups.orderup', 'JLIB_HTML_MOVE_UP', $saveOrder); ?></span>
									<span><?php echo $this->pagination->orderDownIcon($i, $this->pagination->total, true, 'fieldgroups.orderdown', 'JLIB_HTML_MOVE_DOWN', $saveOrder); ?></span>
								<?php elseif ($listDirn == 'desc') : ?>
									<span><?php echo $this->pagination->orderUpIcon($i, true, 'fieldgroups.orderdown', 'JLIB_HTML_MOVE_UP', $saveOrder); ?></span>
									<span><?php echo $this->pagination->orderDownIcon($i, $this->pagination->total, $item->ordering > 2, 'fieldgroups.orderup', 'JLIB_HTML_MOVE_DOWN', $saveOrder); ?></span>
								<?php endif; ?>
							<?php endif; ?>
