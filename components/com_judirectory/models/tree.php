...
class JUDirectoryModelTree extends JUDIRModelList
{
	...
	public static function getDataFeed($categoryId, $ordering = 'listing.title', $direction = 'ASC', $start = 0, $limit = 0)
	{
		$user         = JFactory::getUser();
		$levels       = $user->getAuthorisedViewLevels();
		...
		$query->select('listing.*');
		$query->from('#__judirectory_listings AS listing');
		$query->select('listingxref.cat_id');
		$query->join('', '#__judirectory_listings_xref AS listingxref ON listing.id = listingxref.listing_id');
		$query->select('v.title AS access_title');
		$query->join('LEFT', '#__viewlevels AS v ON listing.access = v.id');
		$query->select('u.name AS creator');
		$query->join('LEFT', '#__users AS u ON listing.created_by = u.id');
		$query->select('m.name AS modifier');
		$query->join('LEFT', '#__users AS m ON listing.modified_by = m.id');
		$query->where('listingxref.cat_id = ' . $categoryId);
		$query->where('listing.published = 1');
		$query->where('listing.approved = 1');
		$query->where('(listing.publish_up = ' . $db->quote($nullDate) . ' OR listing.publish_up <= ' . $db->quote($nowDate) . ')');
		$query->where('(listing.publish_down = ' . $db->quote($nullDate) . ' OR listing.publish_down >= ' . $db->quote($nowDate) . ')');
		$query->where('listing.access IN (' . $levelsString . ')');
		$query->order($ordering . ' ' . $direction);
		if ($limit > 0)
		{
		...
		$listingObjectList = $db->loadObjectList();
