...
<div id="judir-container" class="jubootstrap component judir-container view-listalpha <?php echo $this->pageclass_sfx; ?>">
	...
	if (count($this->listAlpha))
	{
		...
		?>
		<div class="judir-alpha-list clearfix">
			<ul class="pagination">
				<?php
				foreach ($this->listAlpha AS $alpha)
				{
					if ($this->model->checkAlpha($alpha))
					{
						...
						{
							echo '<li class="active"><a href="' . JRoute::_(JUDirectoryHelperRoute::getListAlphaRoute($this->cat_id, $alpha)) . '">' . $alpha . '</a></li>';
						}
						else
						{
							echo '<li><a href="' . JRoute::_(JUDirectoryHelperRoute::getListAlphaRoute($this->cat_id, $alpha)) . '">' . $alpha . '</a></li>';
						}
					}
					else
					{
