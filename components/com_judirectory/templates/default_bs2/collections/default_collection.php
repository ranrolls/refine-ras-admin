...
$item_class_arr[] .= $this->view_mode == 2 ? "span" . $this->collection_bootstrap_columns[$this->col_counter - 1] : "span12";

$item_class = implode(" ", $item_class_arr);

$collection_grid_col = $this->collection_bootstrap_columns[$this->col_counter - 1];
?>
	<div class="<?php echo $item_class; ?>" data-list-class="span12" data-grid-class="span<?php echo $collection_grid_col; ?>">
		<?php
		...
		<?php if ($this->item->can_edit || $this->item->can_delete)
		{
			...
			<div class="private-actions btn-group pull-right">
				<a class="btn btn-small dropdown-toggle" data-toggle="dropdown" href="#"><span class="fa fa-cog"></span> <span class="caret"></span></a>
			...
			if ($this->item->can_vote)
			{
				...
				if ($this->params->get('collection_allow_vote_down', 1))
				{
					if (!$this->item->voted_value)
					{
						...
						$vote_btn_html .= '<button class="vote-up btn btn-mini" title="' . JText::_('COM_JUDIRECTORY_VOTE_UP') . '" onclick="return false;">
												<i class="fa fa-thumbs-o-up"></i>
											</button>';
						$vote_btn_html .= '<button class="vote-down btn btn-mini" title="' . JText::_('COM_JUDIRECTORY_VOTE_DOWN') . '" onclick="return false;">
...
if ((($this->col_counter % $this->collection_columns) == 0) && (($this->index + 1) < count($this->items)))
{
	...
	<div
	class="judir-collection-row <?php echo $this->collection_row_class; ?> judir-collection-row-<?php echo $this->row_counter + 1; ?> row-fluid">
<?php
}
?>