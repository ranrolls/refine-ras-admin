						...
						<div class="comment-text">
									...
									if ($this->commentObj->can_vote)
									{
										...
										if ($this->params->get('allow_vote_down_comment', 1))
										{
											if (!$this->commentObj->voted_value)
											{
												...
												$vote_btn_html .= '<button class="vote-up btn btn-min" title="' . JText::_('COM_JUDIRECTORY_VOTE_UP') . '"
																	onclick="judirVoteComment(\''.$this->commentObj->id.'\', \'up\', \''.JSession::getFormToken().'\', this); return false;">
																	<i class="fa fa-thumbs-o-up"></i>
																</button>';
												$vote_btn_html .= '<button class="vote-down btn btn-mini" title="' . JText::_('COM_JUDIRECTORY_VOTE_DOWN') . '"
							...
							<div class="comment-actions clearfix">
								...
								if (JUDIRPROVERSION && $this->commentObj->can_subscribe)
								{
									...
									{
										?>
										<a class="judir-unsubscribe-comment btn btn-mini" href="<?php echo $this->commentObj->subscribe_link; ?>">
									...
									{
										?>
										<a class="judir-subscribe-comment btn btn-mini" href="<?php echo $this->commentObj->subscribe_link; ?>">
								...
								if ($this->commentObj->can_edit)
								{
									if ($this->commentObj->checked_out)
									{
										if ($this->commentObj->checkout_link)
										{
											...
											echo '<a class="hasTooltip btn btn-mini" title="' . $tooltip . '" href="' . $this->commentObj->checkout_link . '"><i class="fa fa-lock"></i> ' . JText::_('COM_JUDIRECTORY_EDIT_COMMENT') . '</a>';
										}
										else
										{
											echo '<span class="btn btn-mini"><i class="fa fa-lock"></i> ' . JText::_('COM_JUDIRECTORY_EDIT_COMMENT') . '</span>';
										}
									}
									else
									{
									...
									<div id="judir-comment-delete-alert-<?php echo $this->commentObj->id; ?>"
										...
										<div class="modal-footer">
											<a href="<?php echo $this->commentObj->link_delete; ?>"
											        class="btn btn-primary"><?php echo JText::_("COM_JUDIRECTORY_DELETE"); ?>
											</a>
