<?php
/**
 * @version		$Id:router.php 1 2015-06-25Z  $
 * @package		Fb
 * @subpackage 	Router
...
defined('_JEXEC') or die('Restricted access');
  ...
  function FbBuildRoute( &$query )
  {
	...
	{
		$view = $query['view'];
		if (empty($query['Itemid']) || empty($menuItem) || $menuItem->component != 'com_fb')
		{
			$segments[] = $query['view'];
			unset($query['view']);
		}
	...
	if (isset($view) && in_array($view, $listviews))
	{
		...
		if ((isset($query['id']) && ($mId != (int) $query['id'])) || $mView != $view)
		{
			...
			$categories = JCategories::getInstance('Fb', $options);
